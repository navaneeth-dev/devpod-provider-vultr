...
func (cmd *CommandCmd) Run(ctx context.Context, options *options.Options, log log.Logger) error {
	...
	// create client
	instance, err := vultr.NewVultr(options.Token, ctx).GetByName(ctx, options.MachineID)
	if err != nil {
		return err
	...
	// // loop over addresses
	// externalIP := ""
	// for _, network := range instance.Networks.V4 {
	...
	sshClient, err := ssh.NewSSHClient("devpod", instance.MainIP+":22", privateKey)
	if err != nil {
